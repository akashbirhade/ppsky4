...
function calcVarna(rashiBoy: string, rashiGirl: string): number {
  const boy = VARNA_MAP[rashiBoy] || 1;
  const girl = VARNA_MAP[rashiGirl] || 1;
  return boy >= girl ? 1 : 0;
}

function calcVashya(rashiBoy: string, rashiGirl: string): number {
  const boy = VASHYA_MAP[rashiBoy] || 'Nara';
  const girl = VASHYA_MAP[rashiGirl] || 'Nara';
  if (boy === girl) return 2;
  if ((boy === 'Nara' && girl === 'Chatushpad') || (boy === 'Chatushpad' && girl === 'Nara')) return 1;
  return 0;
}

function calcTara(nakshatraBoy: string, nakshatraGirl: string): number {
  const boyIdx = NAKSHATRA_LIST.indexOf(nakshatraBoy as any);
  const girlIdx = NAKSHATRA_LIST.indexOf(nakshatraGirl as any);
  if (boyIdx < 0 || girlIdx < 0) return 1;
  const remainder = ((boyIdx - girlIdx + 27) % 9);
  const favorable = [1, 2, 4, 6, 8];
  return favorable.includes(remainder) ? 3 : 0;
}

function calcYoni(nakshatraBoy: string, nakshatraGirl: string): number {
  ...
  for (const pair of enemies) {
    if ((pair[0] === boy && pair[1] === girl) || (pair[1] === boy && pair[0] === girl)) return 0;
  }
  return 2;
}

function calcGrahaMaitri(rashiBoy: string, rashiGirl: string): number {
  const boyIdx = RASHI_LIST.indexOf(rashiBoy as any);
  const girlIdx = RASHI_LIST.indexOf(rashiGirl as any);
  if (boyIdx < 0 || girlIdx < 0) return 3;
  const diff = Math.abs(boyIdx - girlIdx);
  if (diff <= 1 || diff >= 11) return 5;
  if (diff <= 3 || diff >= 9) return 3;
  return 1;
}

function calcGana(nakshatraBoy: string, nakshatraGirl: string): number {
  ...
  if (boy === 'Manushya' && girl === 'Deva') return 4;
  if (boy === 'Rakshasa' || girl === 'Rakshasa') return 0;
  return 3;
}

function calcBhakoot(rashiBoy: string, rashiGirl: string): number {
  const boyIdx = RASHI_LIST.indexOf(rashiBoy as any);
  const girlIdx = RASHI_LIST.indexOf(rashiGirl as any);
  if (boyIdx < 0 || girlIdx < 0) return 4;
  const diff = ((boyIdx - girlIdx + 12) % 12);
  const unfavorable = [2, 5, 6, 8, 9, 12];
  return unfavorable.includes(diff) ? 0 : 7;
}

function calcNadi(nakshatraBoy: string, nakshatraGirl: string): number {
  const boy = NADI_MAP[nakshatraBoy] || 'Aadi';
  const girl = NADI_MAP[nakshatraGirl] || 'Aadi';
  return boy === girl ? 0 : 8;
}

function isManglik(rashi: string, nakshatra: string): boolean {
...
export const calculateKundali = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (percentage >= 75) recommendation = 'Excellent match! Highly recommended.';
    else if (percentage >= 50) recommendation = 'Good match. Proceed with confidence.';
    else if (percentage >= 33) recommendation = 'Average compatibility. Consider other factors.';
    else recommendation = 'Below average. Discuss with a pandit before proceeding.';

    const manglikBoy = isManglik(boyRashi, boyNakshatra);
    const manglikGirl = isManglik(girlRashi, girlNakshatra);
    let manglikStatus: string;
    if (!manglikBoy && !manglikGirl) manglikStatus = 'Neither is Manglik - No issues';
    else if (manglikBoy && manglikGirl) manglikStatus = 'Both are Manglik - Doshas cancel out';
    else manglikStatus = 'Manglik Dosha present - Remedies recommended';
...
export const getRashiList = async (_req: Request, res: Response) => {
