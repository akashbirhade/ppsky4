...
const COMMUNITIES: Record<string, { id: string; name: string }[]> = {
  hindu: [
...
const SUB_COMMUNITIES: Record<string, { id: string; name: string }[]> = {
  brahmin: [
...
function queryList(
  ...
  req: Request
): { items: { id: string; name: string }[]; total: number; page: number; limit: number } {
...
function setCacheHeaders(res: Response, maxAge = 86400) {
...
router.get('/states', (req, res) => {
  const countryId = (req.query.countryId as string) || 'IN';
  setCacheHeaders(res);
...
router.get('/communities', (req, res) => {
  const religionId = req.query.religionId as string;
  setCacheHeaders(res);
