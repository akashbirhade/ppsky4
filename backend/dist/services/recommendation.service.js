...
function calculateCompatibility(profile, target, preferences, targetPreferences) {
    ...
    if (targetAge >= preferences.minAge && targetAge <= preferences.maxAge) {
        ...
        const diff = Math.min(Math.abs(targetAge - preferences.minAge), Math.abs(targetAge - preferences.maxAge));
    ...
    const distance = haversine(Number(profile.latitude), Number(profile.longitude), Number(target.latitude), Number(target.longitude));
    let locationScore = 0;
    if (distance <= 10)
        locationScore = 15;
    else if (distance <= 25)
...
class RecommendationService {
    async getRecommended(userId, gender, page, limit) {
        const myData = await prisma_1.default.profile.findUnique({
            where: { userId },
        ...
        const candidates = await prisma_1.default.profile.findMany({
            where: {
                user: {
                    gender: opposite,
        ...
        const scored = candidates
            .map((candidate) => ({
            ...candidate,
            compatibilityScore: calculateCompatibility(myData, candidate, myPreferences ?? {}, candidate.user.preferences ?? {}).total,
        }))
            .sort((a, b) => {
    ...
    async getCompatibilityScore(userId, targetUserId) {
        const [myProfile, targetProfile] = await Promise.all([
            prisma_1.default.profile.findUnique({
                where: { userId },
                include: { user: { include: { preferences: true } } },
            }),
            prisma_1.default.profile.findUnique({
        ...
        return calculateCompatibility(myProfile, targetProfile, myProfile.user.preferences ?? {}, targetProfile.user.preferences ?? {});
    }
}
