...
export default function AIChatBot() {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && !isMinimized && (
        <div className="fixed bottom-24 right-6 z-[100] w-[360px] sm:w-[400px] h-[520px] rounded-3xl overflow-hidden flex flex-col animate-scale-in shadow-[0_20px_80px_rgba(0,0,0,0.5),0_0_40px_rgba(147,51,234,0.2)] border border-purple-500/20">
          {/* Header */}
          <div className="bg-gradient-to-r from-purple-900 via-purple-800 to-fuchsia-900 p-4 border-b border-purple-500/20">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-dark-900/95 backdrop-blur-xl">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                <div className={`flex items-end gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ...
                    {msg.role === 'user' ? <User className="h-3 w-3 text-purple-300" /> : <Bot className="h-3 w-3 text-fuchsia-300" />}
                  </div>
                  <div className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                    msg.role === 'user'
                      ? 'bg-purple-600/40 text-white border border-purple-500/20 rounded-br-sm'
          ...
          {/* Quick Actions */}
          {messages.length <= 1 && (
            <div className="px-3 py-2 bg-dark-900/90 border-t border-purple-500/10">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                {["Find matches", "Profile tips", "AI Bio Writer", "Compatibility check", "Red flag alerts", "Reply suggestions", "Kundali matching", "कुंडली मिलान"].map((a, i) => (
                  <button key={i} onClick={() => sendMessage(a)}
                    className="text-xs bg-purple-500/10 text-purple-300 px-3 py-1.5 rounded-full whitespace-nowrap hover:bg-purple-500/20 border border-purple-500/20 transition-all">
                    {a}
                  </button>
                ))}
              </div>
          ...
          {/* Input */}
          <div className="p-3 bg-dark-900/95 border-t border-purple-500/10">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-white/5 rounded-full text-sm text-white placeholder-purple-300/40 focus:outline-none focus:ring-1 focus:ring-purple-500/50 border border-purple-400/10 transition-all"
              />
              <button
                onClick={() => sendMessage()}
                disabled={!input.trim() || isTyping}
                className="w-9 h-9 bg-purple-600 hover:bg-purple-500 disabled:bg-purple-900/50 text-white rounded-full flex items-center justify-center transition-all disabled:opacity-40"
              >
