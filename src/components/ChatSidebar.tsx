...
function ChatAvatar({ name, photo, gender, online, verified }: { name: string; photo?: string; gender?: 'male' | 'female'; online?: boolean; verified?: boolean }) {
  ...
  return (
    <div className="relative shrink-0">
      <div className="w-11 h-11 rounded-full overflow-hidden flex items-center justify-center border border-purple-500/20">
        {!showPlaceholder ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={photo}
...
export default function ChatSidebar() {
  ...
  if (isMinimized) {
    return (
      <button
        ...
        className="hidden xl:flex fixed bottom-6 right-6 z-40 items-center gap-2 px-4 py-2.5 bg-dark-900/95 backdrop-blur-xl border border-purple-500/20 rounded-full shadow-[0_4px_24px_rgba(147,51,234,0.2)] hover:border-purple-500/40 hover:scale-105 transition-all duration-300"
      >
        <MessageCircle className="h-4 w-4 text-purple-400" />
        <span className="text-sm font-medium text-purple-200">Chats</span>
  ...
  return (
    <div className="hidden xl:flex fixed right-0 top-[104px] w-72 h-[calc(100vh-104px)] bg-dark-900/95 backdrop-blur-xl border-l border-purple-500/10 z-40 flex-col">

      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-purple-500/10 bg-white/[0.02]">

        {/* Online Status Dropdown */}
        <div className="relative flex-1" ref={statusMenuRef}>
          <button
            onClick={() => setShowStatusMenu(v => !v)}
            className="flex items-center gap-1.5 hover:bg-purple-500/10 rounded-md px-1 py-0.5 transition-colors w-full"
          >
            <span className={`w-2 h-2 rounded-full shrink-0 ${status.dotClass}`} />
            <span className="text-xs font-medium text-purple-200 flex-1 text-left">{status.label}</span>
            <ChevronDown className={`h-3 w-3 text-purple-400/60 transition-transform ${showStatusMenu ? 'rotate-180' : ''}`} />
          </button>

          {showStatusMenu && (
            <div className="absolute top-full left-0 mt-1 w-40 bg-dark-900 border border-purple-500/20 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden z-50">
              {(Object.entries(STATUS_CONFIG) as [OnlineStatus, typeof STATUS_CONFIG[OnlineStatus]][]).map(([key, cfg]) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs hover:bg-purple-500/10 transition-colors ${onlineStatus === key ? 'text-purple-200' : 'text-purple-300/50'}`}
                >
                  <span className={`w-2 h-2 rounded-full ${cfg.dotClass}`} />
                  {cfg.label}
                  {onlineStatus === key && <Circle className="w-1.5 h-1.5 fill-purple-400 text-purple-400 ml-auto" />}
                </button>
              ))}
            </div>
          )}
        ...
        <button
          ...
          className={`p-1 rounded-full transition-colors ${isMuted ? 'text-red-400 hover:bg-red-500/10' : 'text-purple-400/40 hover:bg-purple-500/10'}`}
        ...
        <button
          ...
          title="Minimize"
        >
          <Minus className="h-3.5 w-3.5 text-purple-400/60" />
        </button>
      </div>

      {/* Search Bar */}
      <div className="px-3 py-2 border-b border-purple-500/5">
        <div className="flex items-center gap-2 bg-white/5 rounded-full px-3 py-1.5 border border-purple-500/10 focus-within:border-purple-500/30 transition-colors">
          <Search className="h-3.5 w-3.5 text-purple-400/40 shrink-0" />
          <input
            type="text"
            value={searchQuery}
          ...
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="text-purple-400/40 hover:text-purple-300 text-xs leading-none">✕</button>
          )}
        </div>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1">
                  <span className="text-sm font-semibold text-purple-100 truncate group-hover:text-white transition-colors">
                    {chatUser.name}
                  </span>
                  <span className="text-[10px] text-purple-300/40 whitespace-nowrap shrink-0">{chatUser.time}</span>
                </div>
                <p className="text-xs text-purple-300/40 truncate mt-0.5">{chatUser.message}</p>
              </div>
              {chatUser.unread > 0 && (
      ...
      {/* Bottom Tabs */}
      <div className="flex border-t border-purple-500/10 bg-white/[0.02]">
        <button
          onClick={() => setActiveTab('alerts')}
          className={`flex-1 py-2 text-[10px] font-medium text-center transition-colors ${activeTab === 'alerts' ? 'text-purple-300 border-t-2 border-purple-400' : 'text-purple-300/30 hover:text-purple-300/60'}`}
        >
          Alerts (31)
        </button>
        <button
          onClick={() => setActiveTab('chats')}
          className={`flex-1 py-2 text-[10px] font-medium text-center transition-colors ${activeTab === 'chats' ? 'text-purple-300 border-t-2 border-purple-400' : 'text-purple-300/30 hover:text-purple-300/60'}`}
        >
          Chats (46)
        </button>
        <button
          onClick={() => setActiveTab('active')}
          className={`flex-1 py-2 text-[10px] font-medium text-center transition-colors ${activeTab === 'active' ? 'text-purple-300 border-t-2 border-purple-400' : 'text-purple-300/30 hover:text-purple-300/60'}`}
        >
