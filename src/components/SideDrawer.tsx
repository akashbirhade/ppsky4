...
export default function SideDrawer() {
  ...
  return (
    <>
      {/* Trigger Button */}
      <button onClick={() => setIsOpen(true)} className="lg:hidden fixed top-[18px] left-4 z-[60] p-2 rounded-xl bg-white/5 border border-purple-500/10 text-purple-300">
      ...
      {/* Drawer */}
      <div className={`fixed top-0 left-0 h-full w-80 z-[201] bg-dark-900/98 border-r border-purple-500/10 transform transition-transform duration-300 overflow-y-auto ${
        ...
        <div className="p-5 border-b border-purple-500/10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setIsOpen(false)} className="p-1.5 rounded-lg text-purple-300/40 hover:text-white hover:bg-white/5 transition-all">
              <X className="h-5 w-5" />
            </button>
          </div>
          {/* User Info */}
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-purple-300/40">ID: {profileId}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-3">
          ...
          <Link href="/checkout?plan=gold" onClick={() => setIsOpen(false)}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-white">Upgrade Now</span>
              <span className="text-[9px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold">70% OFF</span>
            </div>
...
function DrawerLink({ href, icon, label, badge, onClick }: { href: string; icon: React.ReactNode; label: string; badge?: string; onClick: () => void }) {
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-purple-200/70 hover:text-white hover:bg-purple-500/10 transition-all text-sm">
      {icon}
      <span className="flex-1">{label}</span>
      {badge && (
        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${
          badge === 'VIP' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-purple-500/20 text-purple-300 border border-purple-500/30'
        }`}>{badge}</span>
      )}
    </Link>
  )
...
function DrawerButton({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-purple-200/70 hover:text-white hover:bg-purple-500/10 transition-all text-sm text-left">
      {icon}
      <span>{label}</span>
    </button>
  )
