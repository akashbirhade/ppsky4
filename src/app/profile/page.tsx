    ...
    <div className="min-h-screen bg-mesh pt-24 pb-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="glass-card mb-6 p-8 relative overflow-hidden animate-fade-in-up">
          <div className="absolute top-0 right-0 w-48 h-48 bg-purple-600/10 rounded-full blur-[60px]" />
          <div className="relative flex flex-col sm:flex-row items-center gap-6">
            {/* Avatar */}
            <div className="relative group">
              <div className="w-28 h-28 rounded-full bg-gradient-to-br from-purple-500/30 to-fuchsia-500/30 flex items-center justify-center border-2 border-purple-400/30 shadow-[0_0_30px_rgba(147,51,234,0.2)]">
                {photos.length > 0 ? (
                  <img src={photos[0]} alt="Profile" className="w-full h-full rounded-full object-cover" />
                ) : (
                  <User className="h-12 w-12 text-purple-300/60" />
                )}
              </div>
            ...
            <div className="text-center sm:text-left flex-1">
              ...
              <div className="flex items-center gap-2 mt-2 justify-center sm:justify-start">
                <span className="text-[10px] bg-purple-500/20 text-purple-300 px-2.5 py-1 rounded-full border border-purple-500/20 flex items-center gap-1">
            ...
            <div className="text-center">
              <div className="relative w-20 h-20">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-lg font-bold text-white">{completionScore}%</span>
                </div>
              </div>
              <p className="text-[10px] text-purple-300/40 mt-1">Profile Score</p>
            </div>
          </div>
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            <div className="text-center p-3 bg-white/[0.02] rounded-xl border border-purple-500/10">
            ...
            <div className="text-center p-3 bg-white/[0.02] rounded-xl border border-purple-500/10">
              ...
              <p className="text-[10px] text-purple-300/40">Shortlisted</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {activeTab === 'personal' && (
            <div className="glass-card animate-fade-in-up">
              <h2 className="text-lg font-semibold text-white mb-5 flex items-center gap-2">
                <BookOpen className="h-5 w-5 text-purple-400" /> Personal Details
              </h2>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Religion</label>
                ...
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Caste</label>
                  <input type="text" name="caste" value={profile.caste} onChange={handleChange} className="input-field" placeholder="Enter caste" />
                </div>
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Mother Tongue</label>
                  <select name="motherTongue" value={profile.motherTongue} onChange={handleChange} className="input-field">
                    ...
                    {['Hindi','English','Marathi','Tamil','Telugu','Kannada','Gujarati','Punjabi','Bengali','Malayalam','Urdu'].map(l => (
                      <option key={l} value={l} className="bg-dark-900">{l}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Height</label>
                  <select name="height" value={profile.height} onChange={handleChange} className="input-field">
                    ...
                    {["5'0\"","5'1\"","5'2\"","5'3\"","5'4\"","5'5\"","5'6\"","5'7\"","5'8\"","5'9\"","5'10\"","5'11\"","6'0\"","6'1\"","6'2\""].map(h => (
                      <option key={h} value={h} className="bg-dark-900">{h}</option>
                    ))}
                  </select>
                </div>
              </div>
              <div className="mt-4">
                <label className="block text-xs text-purple-200/50 mb-1.5">About Me</label>
          ...
          {activeTab === 'photos' && (
            <div className="glass-card animate-fade-in-up">
              <h2 className="text-lg font-semibold text-white mb-5 flex items-center gap-2">
                <Camera className="h-5 w-5 text-purple-400" /> Photo Gallery
              </h2>
              <p className="text-xs text-purple-300/40 mb-4">Upload 4-6 photos for the best results. Profiles with photos get 10x more responses.</p>
              
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {photos.map((photo, i) => (
                  <div key={i} className="relative aspect-square rounded-2xl overflow-hidden border border-purple-500/20 group">
                    <img src={photo} alt={`Photo ${i+1}`} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button onClick={() => setPhotos(prev => prev.filter((_, idx) => idx !== i))} className="text-white text-xs bg-red-500/80 px-3 py-1.5 rounded-lg">
                        Remove
                      </button>
                    </div>
                    {i === 0 && (
                      <span className="absolute top-2 left-2 text-[9px] bg-purple-600 text-white px-2 py-0.5 rounded-full">Primary</span>
                    )}
                  </div>
                ))}
                
                {/* Upload Button */}
                <label className="aspect-square rounded-2xl border-2 border-dashed border-purple-500/20 flex flex-col items-center justify-center cursor-pointer hover:border-purple-400/40 hover:bg-purple-500/5 transition-all group">
                  <Upload className="h-8 w-8 text-purple-400/40 group-hover:text-purple-300 transition-colors mb-2" />
                  <span className="text-xs text-purple-300/40 group-hover:text-purple-200">Upload Photo</span>
          ...
          {activeTab === 'career' && (
            <div className="glass-card animate-fade-in-up">
              <h2 className="text-lg font-semibold text-white mb-5 flex items-center gap-2">
                <Briefcase className="h-5 w-5 text-purple-400" /> Education & Career
              </h2>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Education</label>
                  <select name="education" value={profile.education} onChange={handleChange} className="input-field">
                    ...
                    {["Bachelor's","Master's","MBA","B.Tech/B.E","M.Tech","MBBS","MD","CA","PhD","Other"].map(e => (
                      <option key={e} value={e} className="bg-dark-900">{e}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Occupation</label>
                ...
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Annual Income</label>
                  <select name="income" value={profile.income} onChange={handleChange} className="input-field">
                    ...
                    {['Below 5 LPA','5-10 LPA','10-15 LPA','15-20 LPA','20-30 LPA','30-50 LPA','50+ LPA'].map(i => (
                      <option key={i} value={i} className="bg-dark-900">{i}</option>
                    ))}
                  </select>
                </div>
              </div>
          ...
          {activeTab === 'preferences' && (
            <div className="glass-card animate-fade-in-up">
              ...
              <p className="text-xs text-purple-300/40 mb-4">Help our AI find better matches by telling us what you&apos;re looking for.</p>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Age Range</label>
                ...
                <div>
                  <label className="block text-xs text-purple-200/50 mb-1.5">Location</label>
          ...
          {activeTab === 'video' && (
            <div className="glass-card animate-fade-in-up">
              <h2 className="text-lg font-semibold text-white mb-5 flex items-center gap-2">
                <Video className="h-5 w-5 text-purple-400" /> Video Introduction
              </h2>
              <p className="text-xs text-purple-300/40 mb-4">Record a short video to introduce yourself. Profiles with videos get 5x more responses!</p>
              
              <div className="border-2 border-dashed border-purple-500/20 rounded-2xl p-10 text-center hover:border-purple-400/40 transition-colors">
                <Video className="h-12 w-12 text-purple-400/30 mx-auto mb-4" />
                <h3 className="text-sm font-semibold text-white mb-2">Upload Your Video Introduction</h3>
                <p className="text-xs text-purple-300/40 mb-4">Maximum 2 minutes • MP4, MOV format</p>
              ...
              <div className="mt-5 p-4 bg-purple-500/5 border border-purple-500/10 rounded-xl">
                <h4 className="text-sm font-medium text-white mb-2 flex items-center gap-2">
                  <Sparkles className="h-4 w-4 text-purple-400" /> Tips for a great video
                </h4>
                <ul className="space-y-2">
                  {[
                    ...
                    <li key={i} className="text-xs text-purple-300/50 flex items-start gap-2">
                      <Star className="h-3 w-3 text-purple-400/60 mt-0.5 shrink-0" />
                      {tip}
                    </li>
                  ))}
                </ul>
