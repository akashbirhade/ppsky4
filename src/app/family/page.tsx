  ...
  return (
    <div className="min-h-screen bg-mesh pt-[104px] pb-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 animate-fade-in-up">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 dark:text-white flex items-center gap-3">
              <Users className="h-8 w-8 text-teal-600 dark:text-purple-400" /> Family Account
            </h1>
            <p className="text-slate-500 dark:text-purple-200/50 mt-2">Manage profiles together with your family members</p>
          </div>
        ...
        {/* Info Banner */}
        <div className="glass-card p-5 mb-8 border-teal-200/50 dark:border-purple-500/20 animate-fade-in-up" style={{ animationDelay: '0.05s', opacity: 0 }}>
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-teal-100/50 dark:bg-purple-500/20 flex items-center justify-center flex-shrink-0">
              <Shield className="h-5 w-5 text-teal-600 dark:text-purple-400" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-1">Family Safety</h3>
              <p className="text-xs text-slate-500 dark:text-purple-200/50">Family members can help manage profiles and review matches. Each member has permission-based access. Only admins can chat or accept interests.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
              <Users className="h-5 w-5 text-teal-600 dark:text-purple-400" /> Family Members ({members.length})
            </h2>
            <div className="space-y-3">
              {members.map((member, i) => (
                <div key={member.id} className="glass-card !p-4 animate-fade-in-up flex items-center gap-4"
                  style={{ animationDelay: `${i * 0.05}s`, opacity: 0 }}>
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500/20 to-fuchsia-500/20 flex items-center justify-center text-xl border border-teal-200/40 dark:border-purple-400/20">
                    {member.avatar}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-purple-200/50">{member.relation} • {member.role}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                      member.role === 'admin' ? 'bg-amber-500/10 text-amber-300 border-amber-500/20' :
                      member.role === 'manager' ? 'bg-teal-50 dark:bg-purple-500/10 text-slate-600 dark:text-purple-300 border-teal-200/50 dark:border-purple-500/20' :
                      'bg-white/5 text-slate-400 dark:text-purple-300/60 border-teal-100 dark:border-purple-500/10'
                    }`}>
            ...
            {/* Managed Profiles */}
            <h2 className="text-lg font-semibold text-slate-800 dark:text-white mt-8 mb-4 flex items-center gap-2">
              <User className="h-5 w-5 text-teal-600 dark:text-purple-400" /> Managed Profiles
            </h2>
            {profiles.map((p, i) => (
              <div key={p.id} className="glass-card !p-5 animate-fade-in-up" style={{ animationDelay: `${(i + 3) * 0.05}s`, opacity: 0 }}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-purple-500/30 to-fuchsia-500/30 flex items-center justify-center border border-teal-200/40 dark:border-purple-400/20">
                      <span className="text-lg font-bold text-slate-500 dark:text-purple-200/60">{p.name[0]}</span>
                    </div>
                    <div>
                      <h3 className="text-base font-semibold text-slate-800 dark:text-white">{p.name}</h3>
                      <p className="text-xs text-slate-500 dark:text-purple-200/50">{p.age} years • {p.gender} • Last active {p.lastActive}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="bg-white/[0.02] rounded-xl p-3 text-center border border-purple-500/5">
                    <p className="text-lg font-bold text-slate-800 dark:text-white">{p.interests}</p>
                    <p className="text-[10px] text-slate-300 dark:text-purple-300/40">Interests</p>
                  </div>
                  <div className="bg-white/[0.02] rounded-xl p-3 text-center border border-purple-500/5">
                    <p className="text-lg font-bold text-slate-800 dark:text-white">{p.views}</p>
                    <p className="text-[10px] text-slate-300 dark:text-purple-300/40">Views</p>
                  </div>
                  <div className="bg-white/[0.02] rounded-xl p-3 text-center border border-purple-500/5">
                    <p className="text-lg font-bold text-slate-800 dark:text-white">86%</p>
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card animate-fade-in-up" style={{ animationDelay: '0.15s', opacity: 0 }}>
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                <Settings className="h-4 w-4 text-teal-600 dark:text-purple-400" /> Role Permissions
              </h3>
              <div className="space-y-3">
                ...
                <div className="p-3 rounded-xl bg-teal-50/50 dark:bg-purple-500/5 border border-teal-100 dark:border-purple-500/10">
                  <div className="flex items-center gap-2 mb-1">
                    <Edit3 className="h-3.5 w-3.5 text-teal-600 dark:text-purple-400" />
                    <span className="text-xs font-semibold text-slate-600 dark:text-purple-300">Manager</span>
                  </div>
                  <p className="text-[10px] text-purple-200/40">Edit profile, shortlist matches, view interests, send interests</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.02] border border-teal-100 dark:border-purple-500/10">
                  <div className="flex items-center gap-2 mb-1">
                    <Eye className="h-3.5 w-3.5 text-slate-400 dark:text-purple-300/60" />
                    <span className="text-xs font-semibold text-slate-500 dark:text-purple-200/70">Viewer</span>
                  </div>
            ...
            <div className="glass-card border-amber-500/20 animate-fade-in-up" style={{ animationDelay: '0.2s', opacity: 0 }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-slate-800 dark:text-white">Family Premium</h3>
              </div>
              <p className="text-xs text-slate-500 dark:text-purple-200/50 mb-4">Upgrade to Family Premium for unlimited members, priority matching, and dedicated relationship advisor.</p>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
            <div className="glass-card max-w-md w-full p-6 animate-scale-in">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-5 flex items-center gap-2">
                <Plus className="h-5 w-5 text-teal-600 dark:text-purple-400" /> Add Family Member
              </h3>
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-slate-700 dark:text-purple-200 mb-2 block">Name *</label>
                ...
                <div>
                  <label className="text-sm text-slate-700 dark:text-purple-200 mb-2 block">Email *</label>
                ...
                <div>
                  <label className="text-sm text-slate-700 dark:text-purple-200 mb-2 block">Relation</label>
                ...
                <div>
                  <label className="text-sm text-slate-700 dark:text-purple-200 mb-2 block">Permission Level</label>
