    ...
    <div className="min-h-screen bg-mesh pt-24 pb-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10 animate-fade-in-up">
          ...
          <h1 className="text-4xl font-bold text-white mb-3">Wedding Marketplace</h1>
          <p className="text-purple-200/50 max-w-lg mx-auto">Find trusted vendors for your dream wedding — venues, photography, catering & more</p>
        </div>

        {/* Search */}
        <div className="glass-card !p-3 mb-6 animate-fade-in-up" style={{ animationDelay: '0.05s', opacity: 0 }}>
          <div className="flex items-center gap-3">
            <Search className="h-5 w-5 text-purple-300/40" />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search vendors by name or city..."
              className="flex-1 bg-transparent text-white text-sm placeholder-purple-300/30 focus:outline-none"
            />
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all border ${
                selectedCategory === cat.id
                  ? 'bg-purple-600/30 text-white border-purple-500/40 shadow-[0_0_15px_rgba(147,51,234,0.2)]'
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-purple-200/50">{filtered.length} vendors found</p>
          <select className="text-xs bg-white/5 text-purple-200/60 border border-purple-500/10 rounded-lg px-3 py-1.5 focus:outline-none">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((vendor, i) => (
            <div key={vendor.id} className="glass-card !p-0 overflow-hidden group hover:border-purple-400/30 transition-all animate-fade-in-up"
              ...
              <div className="h-32 bg-gradient-to-br from-purple-500/10 via-fuchsia-500/5 to-pink-500/10 flex items-center justify-center relative">
                ...
                <div className="absolute top-3 left-3 flex gap-1.5">
                  {vendor.verified && (
                    <span className="bg-blue-500/80 backdrop-blur-sm text-white px-2 py-0.5 rounded-lg text-[9px] font-semibold">✓ Verified</span>
                  )}
                  {vendor.premium && (
                ...
                {/* Price */}
                <div className="absolute bottom-3 right-3 bg-dark-900/80 backdrop-blur-sm px-2.5 py-1 rounded-lg border border-purple-500/20">
                  <span className="text-[11px] font-semibold text-white">{vendor.price}</span>
                </div>
              </div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="text-sm font-semibold text-white group-hover:text-purple-200 transition-colors">{vendor.name}</h3>
                  <div className="flex items-center gap-1">
                    <Star className="h-3 w-3 text-amber-400 fill-amber-400" />
                    <span className="text-xs text-amber-300 font-medium">{vendor.rating}</span>
                    <span className="text-[10px] text-purple-300/40">({vendor.reviews})</span>
                  </div>
                </div>

                <div className="flex items-center gap-1 text-xs text-purple-200/50 mb-2">
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {vendor.tags.map(tag => (
                    <span key={tag} className="text-[9px] bg-purple-500/10 text-purple-300 px-2 py-0.5 rounded-full border border-purple-500/15">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowContact(showContact === vendor.id ? null : vendor.id)}
                    className="flex-1 py-2 text-xs font-medium text-white bg-purple-600/40 rounded-xl border border-purple-500/30 hover:bg-purple-600/60 transition-all flex items-center justify-center gap-1.5">
                    <Phone className="h-3 w-3" /> Contact
                  </button>
                  <button className="py-2 px-3 text-xs text-purple-300 bg-white/5 rounded-xl border border-purple-400/10 hover:bg-white/10 transition-all">
                ...
                {showContact === vendor.id && (
                  <div className="mt-3 p-3 rounded-xl bg-green-500/5 border border-green-500/20 animate-fade-in-up">
                    ...
                    <p className="text-[11px] text-purple-200/60">📞 +91 98765 XXXXX</p>
                    <p className="text-[11px] text-purple-200/60">📧 contact@{vendor.name.toLowerCase().replace(/\s/g, '')}.com</p>
                    <p className="text-[9px] text-purple-300/30 mt-1">* Premium members see full contact details</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-10 glass-card p-8 text-center border-amber-500/20 animate-fade-in-up">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Are you a wedding vendor?</h3>
          <p className="text-sm text-purple-200/50 mb-5">List your business and reach thousands of couples planning their wedding</p>
