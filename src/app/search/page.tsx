  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-xs font-semibold text-slate-700 dark:text-purple-200 bg-teal-100/50 dark:bg-purple-500/20 px-2.5 py-0.5 rounded-full border border-teal-200/50 dark:border-purple-500/20">{inchesToFeet(min)} – {inchesToFeet(max)}</span>
      </div>
      ...
      <div className="relative py-4 border-b border-teal-100 dark:border-purple-500/10">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <form onSubmit={handleSearch} className="glass-card p-4 animate-fade-in-up delay-200" style={{opacity:0}}>
            <div className="flex flex-wrap gap-3">
              <select name="religion" value={filters.religion} onChange={handleChange} className="input-field flex-1 min-w-[130px]">
                ...
                {['Hindu','Muslim','Sikh','Christian','Jain','Buddhist','Parsi'].map(r => (<option key={r} value={r} className="bg-white dark:bg-dark-900">{r}</option>))}
              </select>
              <select name="community" value={filters.community} onChange={handleChange} className="input-field flex-1 min-w-[130px]">
                <option value="" className="bg-white dark:bg-dark-900">Any Community</option>
                {['Brahmin','Rajput','Maratha','Kayastha','Agarwal','Jat','Patel','Reddy','Nair','Iyer','Khatri','Gupta'].map(c => (<option key={c} value={c} className="bg-white dark:bg-dark-900">{c}</option>))}
              </select>
              ...
              <button type="button" onClick={() => setShowFilters(!showFilters)} className="btn-secondary px-4 flex items-center gap-2 text-sm relative">
                ...
                {activeFilterCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-purple-500 text-slate-800 dark:text-white text-[9px] rounded-full flex items-center justify-center">{activeFilterCount}</span>}
              </button>
            </div>

            {/* Profile ID Search & Saved Searches */}
            <div className="flex items-center gap-3 mt-3 pt-3 border-t border-teal-100 dark:border-purple-500/10">
              <div className="flex items-center gap-2 flex-1">
                ...
                <button type="button" onClick={handleProfileIdSearch} className="text-xs text-slate-600 dark:text-purple-300 hover:text-slate-800 dark:text-white transition-colors px-3 py-2 bg-teal-50 dark:bg-purple-500/10 rounded-xl border border-teal-200/50 dark:border-purple-500/20">Go</button>
              </div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={saveCurrentSearch} className="text-xs text-slate-400 dark:text-purple-300/60 hover:text-slate-700 dark:text-purple-200 flex items-center gap-1 px-3 py-2 bg-teal-50/50 dark:bg-purple-500/5 rounded-xl border border-teal-100 dark:border-purple-500/10 transition-all hover:bg-teal-50 dark:bg-purple-500/10">
                  <Save className="h-3 w-3" /> Save
                </button>
                {savedSearches.length > 0 && (
                  <button type="button" onClick={() => setShowSavedSearches(!showSavedSearches)} className="text-xs text-slate-400 dark:text-purple-300/60 hover:text-slate-700 dark:text-purple-200 flex items-center gap-1 px-3 py-2 bg-teal-50/50 dark:bg-purple-500/5 rounded-xl border border-teal-100 dark:border-purple-500/10 transition-all hover:bg-teal-50 dark:bg-purple-500/10">
            ...
            {showSavedSearches && savedSearches.length > 0 && (
              <div className="mt-3 p-3 bg-teal-50/50 dark:bg-purple-500/5 rounded-xl border border-teal-100 dark:border-purple-500/10">
                <p className="text-[10px] text-slate-300 dark:text-purple-300/40 uppercase tracking-wider mb-2">Saved Searches</p>
                <div className="flex flex-wrap gap-2">
                  {savedSearches.map((s, i) => (<span key={i} className="text-xs px-3 py-1.5 bg-teal-50 dark:bg-purple-500/10 text-slate-700 dark:text-purple-200 rounded-full border border-teal-200/50 dark:border-purple-500/20">{s}</span>))}
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-2 mt-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
            <Brain className="h-3.5 w-3.5 text-teal-600 dark:text-purple-400/60 shrink-0" />
            <span className="text-[10px] text-slate-300 dark:text-purple-300/40 shrink-0">AI Suggestions:</span>
            {['Highly Compatible', 'Same City', 'Similar Education', 'New This Week', 'Top Matches'].map(s => (
              <button key={s} className="text-[10px] px-3 py-1 rounded-full bg-teal-50/50 dark:bg-purple-500/5 border border-teal-100 dark:border-purple-500/10 text-slate-400 dark:text-purple-300/50 hover:text-slate-700 dark:text-purple-200 hover:bg-teal-50 dark:bg-purple-500/10 whitespace-nowrap transition-all">{s}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-2">No profiles found</h3>
            <p className="text-sm text-slate-300 dark:text-purple-300/40">Try adjusting your filters</p>
