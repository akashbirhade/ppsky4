    ...
    <div className="bg-dark-900">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className="absolute top-20 left-10 w-72 h-72 bg-purple-600/20 rounded-full blur-[100px] animate-float-slow" />
          <div className="absolute bottom-20 right-10 w-96 h-96 bg-fuchsia-600/15 rounded-full blur-[120px] animate-float" />
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{animationDelay:'4s'}}>
            <div className="animate-orbit" style={{animationDuration:'15s'}}><Sparkles className="h-3 w-3 text-purple-400/40" /></div>
          </div>
        </div>

        <div className="relative z-10 max-w-6xl mx-auto px-4 text-center pt-20">
          <div className="animate-fade-in-up">
            <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-400/20 text-purple-300 px-5 py-2 rounded-full text-sm font-medium mb-8 backdrop-blur-sm">
          ...
          <p className="text-lg sm:text-xl text-purple-200/60 mb-10 max-w-2xl mx-auto animate-fade-in-up delay-200" style={{opacity:0}}>
          ...
          {/* Trust Indicators */}
          <div className="mt-14 flex flex-wrap items-center justify-center gap-6 sm:gap-10 text-sm text-purple-300/50 animate-fade-in-up delay-500" style={{opacity:0}}>
      ...
      <section className="relative py-16 -mt-10">
        <div className="max-w-5xl mx-auto px-4">
          <div className="glass-card p-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              { value: '80L+', label: 'Matches Made', color: 'text-purple-300' },
              { value: '35L+', label: 'Active Profiles', color: 'text-fuchsia-300' },
              { value: '98%', label: 'AI Accuracy', color: 'text-pink-300' },
              { value: '24/7', label: 'AI Support', color: 'text-amber-300' },
            ].map((stat, i) => (
              <div key={i} className="animate-fade-in-up" style={{animationDelay: `${i * 0.1}s`, opacity: 0}}>
                ...
                <div className="text-xs text-purple-300/50 mt-1">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="glass-card group hover:-translate-y-2 transition-all duration-500">
                ...
                <p className="text-sm text-purple-200/50 leading-relaxed">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {[
              ...
              <div key={i} className="text-center relative">
                <div className="relative inline-block mb-4">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 flex items-center justify-center text-2xl shadow-[0_0_30px_rgba(147,51,234,0.3)]">
                    {item.icon}
                  </div>
                  <span className="absolute -top-2 -right-2 text-[10px] font-bold text-purple-400 bg-dark-900 px-1.5 py-0.5 rounded-full border border-purple-500/30">
                    {item.step}
                  </span>
                </div>
                <h3 className="font-semibold text-white mb-1">{item.title}</h3>
                <p className="text-sm text-purple-300/50">{item.desc}</p>
                {i < 3 && <div className="hidden md:block absolute top-8 left-[60%] w-[80%] h-px bg-gradient-to-r from-purple-500/30 to-transparent" />}
              </div>
            ))}
          </div>
        </div>
            ...
            {[
              ...
              <div key={i} className="glass-card group">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-500/20 to-purple-500/20 flex items-center justify-center border border-pink-500/20">
                    <HalfHeart className="h-5 w-5 group-hover:animate-heartbeat" />
                  </div>
                  <div>
                    <h4 className="font-semibold text-white text-sm">{story.names}</h4>
                    <p className="text-[10px] text-purple-300/50">{story.city} • {story.score} Match</p>
                  </div>
                </div>
                <p className="text-sm text-purple-200/60 italic leading-relaxed">&ldquo;{story.story}&rdquo;</p>
              </div>
            ))}
          </div>
      ...
      {/* CTA Section */}
      <section className="py-24 relative">
        <div className="absolute inset-0 bg-gradient-to-r from-purple-600/10 via-fuchsia-600/5 to-purple-600/10" />
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          <div className="glass-card p-12 border-purple-400/20">
            <HalfHeart className="h-12 w-12 mx-auto mb-6 animate-heartbeat" />
            <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
              Your Perfect Match is Waiting
            </h2>
            <p className="text-lg text-purple-200/50 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-bold text-white">Soulmate Sync</span>
              </div>
              <p className="text-xs text-purple-300/40">India&apos;s #1 AI Matchmaking Platform</p>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-white mb-3">Quick Links</h4>
              <div className="space-y-2">
                <Link href="/search" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Search Profiles</Link>
                <Link href="/premium" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Premium Plans</Link>
                <Link href="/success-stories" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Success Stories</Link>
              </div>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-white mb-3">Safety</h4>
              <div className="space-y-2">
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Safety Tips</a>
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Privacy Policy</a>
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Terms of Use</a>
              </div>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-white mb-3">Support</h4>
              <div className="space-y-2">
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Help Center</a>
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Contact Us</a>
                <a href="#" className="block text-xs text-purple-300/50 hover:text-purple-200 transition-colors">Report Issue</a>
              </div>
            </div>
          </div>
          <div className="border-t border-purple-500/10 pt-6 text-center">
            <p className="text-xs text-purple-400/30">© 2024-2026 Soulmate Sync. Made with 💜 in India</p>
          </div>
        </div>
      </footer>
    </div>
